...
import { act, getAllByRole, getByRole, getByText, screen, waitFor, waitForElementToBeRemoved } from '@testing-library/react'
...
describe('Coin page', () => {
  ...
  describe('When it mounts by default with a valid id param', () => {
    ...
    it('Should display correctly formatted asset stats', async () => {
      ...
      const stats = await screen.findByRole('list', { name: `${fakeAsset.name} stats` })
    ...
    it('Should render correctly a table with all fetched asset markets', async () => {
      await renderByDefault()
      const marketsTable = await screen.findByRole('table', { name: /markets table/i })
    ...
    describe('When user clicks on time range buttons', () => {
      it('Should render an image with corresponding time range history chart', async () => {
        ...
        const buttons = await screen.findByRole('generic', { name: /select an interval/i })
